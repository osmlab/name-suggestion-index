...
export function idgen(item, tkv, locationID) {
  ...
  let osmtags;
  if (t === 'transit') {
    osmtags = ['name', 'name:en', 'name:\\w+-Latn', 'network', 'network:en', 'network:\\w+-Latn', 'operator', 'operator:en', 'operator:\\w+-Latn', 'brand', 'brand:en', 'brand:\\w+-Latn'];
  } else if (t === 'operators') {
    osmtags = ['operator', 'operator:en', 'operator:\\w+-Latn', 'name', 'name:en', 'name:\\w+-Latn', 'brand', 'brand:en', 'brand:\\w+-Latn', 'network', 'network:en', 'network:\\w+-Latn'];
  } else if (t === 'flags') {
    osmtags = ['flag:name', 'subject'];
  } else {  // brands
    osmtags = ['name', 'name:en', 'name:\\w+-Latn', 'brand', 'brand:en', 'brand:\\w+-Latn', 'operator', 'operator:en', 'operator:\\w+-Latn', 'network', 'network:en', 'network:\\w+-Latn'];
  }

  // First attempt, pick a name that matches /^\w+$/
  for (let i = 0; i < osmtags.length; i++) {
    let tryname;
    if ( osmtags[i].endsWith(':\\w+-Latn') ) {
      let keylist = [];
