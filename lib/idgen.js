...
export function idgen(item, tkv, locationID) {
  ...
  let osmtags;
  if (t === 'transit') {
    osmtags = ['name', 'name:en', 'name:\\w+-Latn(-\\w+)?', 'network', 'network:en', 'network:\\w+-Latn(-\\w+)?', 'operator', 'operator:en', 'operator:\\w+-Latn(-\\w+)?', 'brand', 'brand:en', 'brand:\\w+-Latn(-\\w+)?'];
  } else if (t === 'operators') {
    osmtags = ['operator', 'operator:en', 'operator:\\w+-Latn(-\\w+)?', 'name', 'name:en', 'name:\\w+-Latn(-\\w+)?', 'brand', 'brand:en', 'brand:\\w+-Latn(-\\w+)?', 'network', 'network:en', 'network:\\w+-Latn(-\\w+)?'];
  } else if (t === 'flags') {
    osmtags = ['flag:name', 'subject'];
  } else {  // brands
    osmtags = ['name', 'name:en', 'name:\\w+-Latn(-\\w+)?', 'brand', 'brand:en', 'brand:\\w+-Latn(-\\w+)?', 'operator', 'operator:en', 'operator:\\w+-Latn(-\\w+)?', 'network', 'network:en', 'network:\\w+-Latn(-\\w+)?'];
  }

  // First attempt, pick a name that matches /^\w+$/
  for (let i = 0; i < osmtags.length; i++) {
    let tryname;
    if ( osmtags[i].includes(':\\w+-Latn') ) {
      let keylist = [];
