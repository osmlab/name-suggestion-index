...
exports.readLegacy = (tree, cache, loco) => {
  cache = cache || { path: {}, id: {} };

  const START = '🏗   ' + colors.yellow(`Reading legacy ${tree}`);
  const END = '👍  ' + colors.green(`legacy ${tree} loaded`);
  console.log('');
  console.log(START);
  console.time(END);
  ...
  glob.sync(`./${tree}/**/*.json`).forEach(file => {
    ...
    Object.keys(json).forEach(kvnd => {
      let item = json[kvnd];
      const kvndparts = kvnd.split('|', 2);     // kvnd = "key/value|name~disambiguator"
      const kv = kvndparts[0];
      const nd = kvndparts[1];
      ...
      // add new properties, remove nomatch
      item.displayName = nd.replace('~', ' ');
      item.oldid = kvnd;
      delete item.nomatch;
      ...
      cache.path[tkv].push(item);
      // NOTE: don't merge into `cache.id` as the ids won't be unique yet!
      // (merging `identifiers` into `master` will take care of this)
    });
  });
