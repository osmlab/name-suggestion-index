  ...
  Object.keys(trees).forEach(t => {
    ...
    paths.forEach(tkv => {
      ...
      templateItems = templateItems
        .sort((a, b) => a.templateSource.localeCompare(b.templateSource, 'en-US'))   // sort templateItems by templateSource
        .map(item => {
          itemCount++;

          // clean templateInclude/templateExclude
          if (item.templateInclude) {
            item.templateInclude = item.templateInclude.map(s => s.toLowerCase()).sort();
          }
          if (item.templateExclude) {
            item.templateExclude = item.templateExclude.map(s => s.toLowerCase()).sort();
          }
      ...
      normalItems = normalItems
        ...
        .sort((a, b) => a.displayName.localeCompare(b.displayName, 'en-US'))   // sort normalItems by displayName
        .map(item => {
          itemCount++;
          // clean displayName
