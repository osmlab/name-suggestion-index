...
module.exports = () => {
  ...
  matcher.buildMatchIndex = (data) => {
    ...
    Object.keys(data).forEach(tkv => {
      ...
      if (!branch) {
        branch = { primary: new Map(), alternate: new Map() };
  ...
  matcher.match = (k, v, n, loc) => {
    ...
    function gatherMatches(which) {
      ...
      const kv = `${k}/${v}`;
      let didMatch = tryMatch(which, kv, nsimple);
      if (didMatch) return;
      ...
      for (let mg in matchGroups) {
        ...
        for (let i = 0; i < matchGroup.length; i++) {
          ...
          didMatch = tryMatch(which, otherkv, nsimple);
          if (didMatch) return;
        }
      }
    }
    ...
    function tryMatch(which, kv, nsimple) {
      const branch = _matchIndex.get(kv);
      if (!branch) return;
