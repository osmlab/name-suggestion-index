...
module.exports = () => {
  ...
  matcher.buildLocationIndex = (all, loco) => {
    ...
    Object.keys(all).forEach(tkv => {
      ...
      items.forEach(item => {
        let resolved = loco.resolveLocationSet(item.locationSet);
        // important: always use the locationSet `id` (`+[Q30]`), not the feature id (`Q30`)
        resolved.feature.id = resolved.id;
        resolved.feature.properties.id = resolved.id;
        locationSets[resolved.id] = resolved.feature;
      });
    });
