...
export class Matcher {
  ...
  constructor() {
    ...
    this.matchIndexPromise = undefined;
    ...
    this.locationIndexPromise = undefined;
  ...
  // }
  //
  async buildMatchIndex(data) {
    const that = this;
    if (that.matchIndexPromise) {
      return that.matchIndexPromise;
    } else {
      return that.matchIndexPromise = new Promise(resolve => {
        setTimeout(() => resolve(that._buildMatchIndex(data)), 0);  // enqueue task
      });
    }
  }

  // `_buildMatchIndex()` internal
  async _buildMatchIndex(data) {
    const that = this;
    if (that.matchIndex) return that.matchIndex;  // it was built already
    that.matchIndex = new Map();

    let categories = Object.keys(data);
    while (categories.length) {   // process 5 categories at a time
      await Promise.all( categories.splice(-5).map(tkv => _indexCategory(tkv)) );
    }
    return that.matchIndex;


    // Insert a single category into the index
    function _indexCategory(tkv) {
      ...
      items.forEach(item => {
        ...
        Object.keys(item.tags).forEach(osmkey => {
          ...
          if (primaryName.test(osmkey)) {
            kvTags.forEach(kv => _indexName('primary', kv, simplify(osmvalue), item.id));
          } else if (alternateName.test(osmkey)) {
            kvTags.forEach(kv => _indexName('alternate', kv, simplify(osmvalue), item.id));
          }
        });
        ...
        (item.matchNames || []).forEach(matchName => {
          ...
          kvTags.forEach(kv => {
            ...
            if (!inPrimary && !inAlternate) {
              _indexName('alternate', kv, nsimple, item.id);
    ...
    function _indexName(which, kv, nsimple, itemID) {
      if (!nsimple)  return;
  ...
  // }
  //
  async buildLocationIndex(data, loco) {
    const that = this;
    if (that.locationIndexPromise) {
      return that.locationIndexPromise;
    } else {
      return that.locationIndexPromise = new Promise(resolve => {
        setTimeout(() => resolve(that._buildLocationIndex(data, loco)), 0);  // enqueue task
      });
    }
  }

  // `_buildLocationIndex()` internal
  async _buildLocationIndex(data, loco) {
    const that = this;
    if (that.locationIndex) return that.locationIndex;   // it was built already
    ...
    function _indexCategory(tkv) {
      ...
      items.forEach(item => {
        ...
        let feature = JSON.parse(JSON.stringify(resolved.feature));  // clone deep
