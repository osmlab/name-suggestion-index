...
* ⚠️  Bump to [`@rapideditor/location-conflation`](https://github.com/rapideditor/location-conflation) v3 — downstream consumers must upgrade simultaneously
  * Consumers who call `matcher.buildLocationIndex(data, loco)` should now pass in their own configured `LocationConflation` instance so indexing and lookups share the same cache
    * `loco` is optional — if omitted, the matcher creates a bare instance internally
    * Eliminates the need for downstream apps (e.g. Rapid) to monkey-patch the matcher to share a `LocationConflation` cache
  * `Matcher` delegates to the supplied `LocationConflation` via its new `registerLocationSets` / `locationSetsAt` / `getLocationSetArea` APIs
    * Removed public fields `itemLocation`, `locationSets`, `locationIndex` — previously used by downstream apps, now unnecessary
    * Added `itemLocationSetID` map (item id → locationSetID string)
  * Removed the `which-polygon` dependency — location lookups are now handled inside `LocationConflation` v3
* Public types now re-export several types from `@rapideditor/location-conflation` for consumer convenience (`LocoFeature`, `LocoProperties`, `HasLocationSet`, `LocationSetID`, `Vec2`, etc.)
* `NsiItem` now `extends HasLocationSet` from `@rapideditor/location-conflation`
* New: `buildIDPresets` and `buildJOSMPresets` are now exported from `name-suggestion-index` as pure, in-memory functions
  * Extracted from `scripts/dist.ts` into `lib/presets_id.ts` and `lib/presets_josm.ts`
  * Take their inputs (NSI data, source iD presets, wikidata, dissolutions) as arguments — no file I/O, no console output, no input mutation
  * Suitable for downstream apps that fetch NSI data on-the-fly and want to generate presets in the browser
  * `dist.ts` now wraps these functions for the file-writing CLI behavior
* ⚠️  Type renames in `lib/types.ts` to clarify that several shapes describe the *generated `dist/*.json` files* rather than the *source `config/*.json` files*:
  * `TreesConfig` → `NsiTreesJSON`
  * `MatchGroupsConfig` → `NsiMatchGroupsJSON`
  * `GenericWordsConfig` → `NsiGenericWordsJSON`
  * `ReplacementsConfig` → `NsiReplacementsJSON`
  * `NsiTreeConfig` → `NsiTreeProperties`
  * `Dissolution` → `WikidataDissolution`
  * `RapidPreset` → `IDPreset`
* New types added to `lib/types.ts`: `NsiPath` (alias for the `tree/key/value` string), `NsiJSON`, `NsiWikidataJSON`, `NsiWarningsJSON`, `WikidataEntry`, `WikidataLogos`, `WikidataIdentities`, `WikidataMap`, `DissolvedMap`, `WikidataWarning`, `TaginfoItem`, `TaginfoJSON`
