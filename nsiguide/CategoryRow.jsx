...
import { faLock, faUnlock } from '@fortawesome/free-solid-svg-icons'; // SSL local and Unlock icon.
...
{ color:green; fill-color:green; }
  ...
  if (t === 'flags') {
    ...
    return (
      <tr className={rowClasses.join(' ') || null}>
        ...
        <td className='tags'>
          <pre className='tags' dangerouslySetInnerHTML={highlight(filters.tt, displayTags(tags))} />
        </td>
    ...
    return (
      val && (
        <a target='_blank' href={href} title={title}>
          <code dangerouslySetInnerHTML={highlight(filters.cc, val)} />
        </a>
      )
    );
  }
  ...
  function logo(src) {
    return src && <img className='logo' src={src} />;
  }
  ...
  function siteLink(href) {
    ...
    FAsecure = (
      <span title='ssl web site'>
        <FontAwesomeIcon icon={faLock} size='lg' />
      </span>
    );
    FAinsecure = (
      <span title='non-ssl web site'>
...
`;
    ...
    if (item.matchNames || item.matchTags || item.note || item.preserveTags || item.issues || item.fromTemplate) result += '<hr/>';
    ...
    if (item.fromTemplate) {
      ...
      if (t === 'brands' && k === 'advertising' && v === 'totem') {
        url = '/index.html?t=brands&amp;k=amenity&amp;v=fuel';
        linktext = '/brands/amenity/fuel.json';
      }

      if (t === 'brands' && k === 'amenity' && v === 'atm') {
        url = '/index.html?t=brands&amp;k=amenity&amp;v=bank';
        linktext = '/brands/amenity/bank.json';
      }

      if (t === 'brands' && k === 'man_made' && v === 'charge_point') {
        url = '/index.html?t=brands&amp;k=amenity&amp;v=charging_station';
        linktext = '/brands/amenity/charging_station.json';
      }

      if (t === 'brands' && k === 'shop' && v === 'car_repair') {
        url = '/index.html?t=brands&amp;k=shop&amp;v=car';
        linktext = '/brands/shop/car.json';
      }

      /* Operators */
      if (t === 'operators' && k === 'leisure' && v === 'nature_reserve') {
        url = '/index.html?t=operators&amp;k=leisure&amp;v=park';
        linktext = '/operators/leisure/park.json';
      }

      if (t === 'operators' && k === 'man_made' && v === 'charge_point') {
        url = '/index.html?t=operators&amp;k=amenity&amp;v=charging_station';
        linktext = '/operators/amenity/charging_station.json';
      }
      if (t === 'operators' && k === 'man_made' && (v === 'pumping_station' || v === 'reservoir_covered')) {
        url = '/index.html?t=operators&amp;k=office&amp;v=water_utility';
        linktext = '/operators/office/water_utility.json';
      }
      if (t === 'operators' && k === 'man_made' && v === 'tower') {
        url = '/index.html?t=operators&amp;k=man_made&amp;v=mast';
        linktext = '/operators/man_made/mast.json';
      }
      ...
      if (t === 'operators' && k === 'pipeline' && v === 'substation') {
        url = '/index.html?t=operators&amp;k=man_made&amp;v=pipeline';
        linktext = 'operators/man_made/pipeline.json';
      }

      if (t === 'operators' && k === 'power' && (v === 'minor_line' || v === 'pole' || v === 'tower')) {
        url = '/index.html?t=operators&amp;k=power&amp;v=line';
        linktext = '/operators/power/line.json';
      }
      if (t === 'operators' && k === 'power' && v === 'transformer') {
        url = '/index.html?t=operators&amp;k=power&amp;v=substation';
        linktext = '/operators/power/substation.json';
      }

      /* Transit */
      if (t === 'transit' && k === 'amenity' && v === 'ferry_terminal') {
        url = '/index.html?t=transit&amp;k=route&amp;v=ferry';
        linktext = '/transit/route/ferry.json';
      }

      if (t === 'transit' && k === 'highway' && v === 'bus_stop') {
        url = '/index.html?t=transit&amp;k=route&amp;v=bus';
        linktext = '/transit/route/bus.json';
      }

      if (t === 'transit' && k === 'public_transport' && v === 'station_light_rail') {
        url = '/index.html?t=transit&amp;k=route&amp;v=light_rail';
        linktext = '/transit/route/light_rail.json';
      }
      if (t === 'transit' && k === 'public_transport' && v === 'station_subway') {
        url = '/index.html?t=transit&amp;k=route&amp;v=light_rail';
        linktext = '/transit/route/subway.json';
      }

      if (t === 'transit' && k === 'railway' && v === 'station') {
        url = '/index.html?t=transit&amp;k=route&amp;v=train';
        linktext = '/transit/route/train.json';
      }
      if (t === 'transit' && k === 'railway' && v === 'tram_stop') {
        url = '/index.html?t=transit&amp;k=route&amp;v=tram';
        linktext = '/transit/route/tram.json';
      }
    ...
    if (item.matchNames) result += '<strong>matchNames</strong>:<br/>' + item.matchNames + '<br/>';
    if (item.matchTags) result += '<strong>matchTags</strong>:<br/>' + item.matchTags + '<br/>';
    if (item.note) result += '<strong>Note</strong>:<br/>' + item.note + '<br/>';
    if (item.preserveTags) result += '<strong>preserveTags</strong>:<br/>' + item.preserveTags + '<br/>';
    if (item.issues) {
